...
class Select(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Integration) -> None:
        if self.values[0] == "Event interval":
            ...
            DB.update(self=self, table='staffing', where=['title'], value={'title': self.title}, columns=['week_interval', 'date'], values={'week_interval': week_int, 'date': dates[0]})
            ...
            DB.update(self=self, table='staffing', where=['title'], value={'title': self.title}, columns=['description'], values={'description': newdescription})
            await StaffingAsync._updatemessage(self=self, title=self.title)
            ...
            await interaction.response.defer()
            check_pos = DB.select(table="positions", columns=['user'], where=['title'], value={'title': self.title}, amount='all')
            update_ok = True
            ...
            if update_ok == True:
                ...
                positions = DB.select(table='positions', columns=['position'], where=['type', 'title'], value={'type': section, 'title': self.title})
                ...
                DB.update(self=self, table='staffing', where=['title'], value={'title': self.title}, columns=[columns[int(section)]], values={columns[int(section)]: section_title})
                if positions != 'None':
                    DB.delete(self=self, table='positions', where=['type', 'title'], value={'type' : section, 'title' : self.title})

                if section_title != 'None':
                    for position in section_pos:
                        DB.insert(self=self, table="positions", columns=['position', 'user', 'type', 'title'], values=[position, "", section, self.title])
            ...
            DB.update(self=self, table='staffing', where=['title'], value={'title': self.title}, columns=['restrict_bookings'], values={'restrict_bookings': restrict_bookings[restriction]})
            ...
            if confirm_delete == self.title:
                event = DB.select(table='staffing', columns=['channel_id', 'message_id'], where=['title'], value={'title': self.title})
                channel_id = event[0]
                message_id = event[1]
                channel = self.bot.get_channel(int(channel_id))
                message = await channel.fetch_message(int(message_id))
                await message.delete()
                DB.delete(self=self, table='staffing', where=['title'], value={ 'title' : self.title, })
                DB.delete(self=self, table='positions', where=['title'], value={ 'title' : self.title, })
