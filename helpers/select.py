...
class Select(discord.ui.Select):
    def __init__(self, id=None, ctx=None, bot=None) -> None:
        options = [
            discord.SelectOption(
                ...
                emoji='🗓️',
                description='Update the event interval',
            ),
            discord.SelectOption(
                label='Staffing message',
                emoji='💬',
                description='Update the staffing message',
            ),
            discord.SelectOption(
                label='Sections & Positions',
                emoji='📰',
                description='Update the different section and positions',
            ),
            discord.SelectOption(
                label='Booking restriction',
                emoji='📅',
                description='Change booking restriction',
            ),
            discord.SelectOption(
                label='Delete Staffing', emoji='❌', description='Delete Staffing'
            ),
        ]
        super().__init__(
    ...
    async def callback(self, interaction: discord.Integration) -> None:
        title = DB.select(
            table='staffing', columns=['title'], where=['id'], value={'id': self.id}
        )[0]
        if self.values[0] == 'Event interval':
            ...
            DB.update(
                ...
                where=['id'],
                value={'id': self.id},
                columns=['week_interval', 'date'],
                values={'week_interval': week_int, 'date': dates[0]},
            )
            await StaffingAsync._updatemessage(self=self, id=self.id)
            await interaction.followup.send(
                f'Event week interval updated to - `{week_int}`'
            )
            ...
            await StaffingAsync._updatemessage(self=self, id=self.id)
            await interaction.followup.send(
                f'Event description/staffing message has been updated to:\n{newdescription}'
            )
            ...
            if update_ok is True:
                ...
                section_title = await StaffingAsync._setup_section(
                    self, self.ctx, section
                )
                positions = DB.select(
                    table='positions',
                    columns=['position'],
                    where=['type', 'event'],
                    value={'type': section, 'event': self.id},
                )
                if section_title != 'None':
                    section_pos = await StaffingAsync._setup_section_pos(
                        self, self.ctx, section_title
                    )
                columns = {
                    1: 'section_1_title',
                    2: 'section_2_title',
                ...
                DB.update(
                    ...
                    where=['id'],
                    value={'id': self.id},
                    columns=[columns[int(section)]],
                    values={columns[int(section)]: section_title},
                )
                if positions != 'None':
                    DB.delete(
                        self=self,
                ...
                if section_title != 'None':
                    for item in section_pos:
                        ...
                        DB.insert(
                            ...
                            values=[
                                section_pos[item]['position'],
                                '',
                                section,
                                local[section_pos[item]['local_booking']],
                                section_pos[item]['start_time'],
                                section_pos[item]['end_time'],
                                self.id,
                            ],
                        )

                    formatted_pos_data = '\n'.join(position for position in section_pos)
                    await interaction.followup.send(
                        f'Section Title updated to `{section_title}`\n\nPositions updated to:\n{formatted_pos_data}'
                    )
                else:
                ...
                await interaction.followup.send(
                    'Update is not OK. There are still active bookings for this event.'
                )
            ...
            DB.update(
                ...
                where=['id'],
                value={'id': self.id},
                columns=['restrict_bookings'],
                values={'restrict_bookings': restrict_bookings[restriction]},
            )
            ...
            event = DB.select(
                table='staffing',
                columns=['title', 'channel_id', 'message_id'],
                where=['id'],
                value={'id': self.id},
            )
            if confirm_delete == event[0]:
                ...
                DB.delete(
                    self=self, table='staffing', where=['id'], value={'id': self.id}
                )
                DB.delete(
                    self=self,
                    table='positions',
                ...
                await interaction.followup.send(
                    f'Staffing for `{event[0]}` has been deleted'
                )
            elif confirm_delete == 'CANCEL':
                await interaction.followup.send(
                    f'Deletion of `{event[0]}` has been cancelled.'
                )
