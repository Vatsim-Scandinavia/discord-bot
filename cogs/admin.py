...
class AdminCog(commands.Cog):
    ...
    async def autocomplete_loaded_cogs(self, interaction: discord.Interaction, current: str):
        ...
        return [
            ...
            for cog in loaded_cogs if current.lower() in cog.lower()
        ]

    async def autocomplete_unloaded_cogs(self, interaction: discord.Interaction, current: str):
        ...
        return [
            app_commands.Choice(name=cog, value=cog)
            for cog in unloaded_cogs if current.lower() in cog.lower()
        ]
    ...
    @app_commands.command(name="unload", description="Command which Unloads an extention")
    @app_commands.checks.has_any_role(*config.STAFF_ROLES)
    ...
    @app_commands.command(name="reload", description="Command which Reloads an extention")
    @app_commands.checks.has_any_role(*config.STAFF_ROLES)
    @app_commands.autocomplete(cog=autocomplete_loaded_cogs)
    ...
    async def cogs(self, interaction: discord.Interaction):
        ...
        fields = [{'name': key, 'value': config.COGS_LOAD[key]} for key in config.COGS_LOAD]
